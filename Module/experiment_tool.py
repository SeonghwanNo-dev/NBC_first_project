...
class ExperimentTool:
    ...
    def __init__(self, path_to_store: str, project_name: str, config: Dict[str, Any]):
        ...
        Method:
            def _save_config(self) -> None: 현재 실험 설정을 JSON 파일로 저장합니다.
            log(self, metrics: Dict[str, Any]) -> None: 학습 과정에서 변화하는 Loss, Accuracy 같은 스칼라 지표를 기록합니다.
            def d_log(self, dataset: pd.DataFrame, name: str) -> str: 실험에 사용된 데이터셋을 CSV 형태로 저장합니다.
            def w_log(self, model: nn.Module, name: str) -> str: 학습된 모델의 가중치(Weights)를 PyTorch .pth 파일로 저장합니다.
        """
